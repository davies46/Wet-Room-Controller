...
void Adafruit_BMP085_Unified::readCoefficients(void) {
	...
	_bmp085Mode = 0;
#else
	readS16(Coefs::BMP085_REGISTER_CAL_AC1, &_bmp085_coeffs.ac1);
	readS16(Coefs::BMP085_REGISTER_CAL_AC2, &_bmp085_coeffs.ac2);
	readS16(Coefs::BMP085_REGISTER_CAL_AC3, &_bmp085_coeffs.ac3);
	read16(Coefs::BMP085_REGISTER_CAL_AC4, &_bmp085_coeffs.ac4);
	read16(Coefs::BMP085_REGISTER_CAL_AC5, &_bmp085_coeffs.ac5);
	read16(Coefs::BMP085_REGISTER_CAL_AC6, &_bmp085_coeffs.ac6);
	readS16(Coefs::BMP085_REGISTER_CAL_B1, &_bmp085_coeffs.b1);
	readS16(Coefs::BMP085_REGISTER_CAL_B2, &_bmp085_coeffs.b2);
	readS16(Coefs::BMP085_REGISTER_CAL_MB, &_bmp085_coeffs.mb);
	readS16(Coefs::BMP085_REGISTER_CAL_MC, &_bmp085_coeffs.mc);
	readS16(Coefs::BMP085_REGISTER_CAL_MD, &_bmp085_coeffs.md);
#endif
}
...
#else
	uint16_t t;
	writeCommand(Coefs::BMP085_REGISTER_CONTROL, Coefs::BMP085_REGISTER_READTEMPCMD);
	delay(5);
	read16(Coefs::BMP085_REGISTER_TEMPDATA, &t);
	*temperature = t;
#endif
}
...
void Adafruit_BMP085_Unified::readRawPressure(int32_t *pressure) {
	...
	*pressure = 23843;
	...
	writeCommand(Coefs::BMP085_REGISTER_CONTROL, Coefs::BMP085_REGISTER_READPRESSURECMD + (_bmp085Mode << 6));
	switch (_bmp085Mode) {
	case Coefs::BMP085_MODE_ULTRALOWPOWER:
		delay(5);
		break;
	case Coefs::BMP085_MODE_STANDARD:
		delay(8);
		break;
	case Coefs::BMP085_MODE_HIGHRES:
		delay(14);
		break;
	case Coefs::BMP085_MODE_ULTRAHIGHRES:
	default:
		delay(26);
		break;
	}

	read16(Coefs::BMP085_REGISTER_PRESSUREDATA, &p16);
	p32 = (uint32_t) p16 << 8;
	read8(Coefs::BMP085_REGISTER_PRESSUREDATA + 2, &p8);
	p32 += p8;
	p32 >>= (8 - _bmp085Mode);
 ...
 */
/**************************************************************************/
bool Adafruit_BMP085_Unified::begin(Coefs::bmp085_mode_t mode) {
	// Enable I2C
	Wire.begin();

	/* Mode boundary check */
	if ((mode > Coefs::BMP085_MODE_ULTRAHIGHRES) || (mode < 0)) {
		mode = Coefs::BMP085_MODE_ULTRAHIGHRES;
	}

	/* Make sure we have the right device */
	uint8_t id;
	read8(Coefs::BMP085_REGISTER_CHIPID, &id);
	if (id != 0x55) {
		return false;
	}
...
void Adafruit_BMP085_Unified::getPressureAndTemp(float *pressure, float *temp) {
	int32_t up = 0, compp = 0;
	int32_t x1, x2, b5, b6, x3, b3, p;
	uint32_t b4, b7;
...
//void Adafruit_BMP085_Unified::getPressure(int32_t ut, float *pressure) {
//	int32_t up = 0, compp = 0;
//	int32_t x1, x2, b5, b6, x3, b3, p;
//	uint32_t b4, b7;
//
...
void Adafruit_BMP085_Unified::getPressure(int32_t b5, float *pressure) {
	int32_t up = 0, compp = 0;
...
float Adafruit_BMP085_Unified::getStoredTemperature() {
...
//void Adafruit_BMP085_Unified::rawTemperature(int32_t *ut) {
//	readRawTemperature(ut);
//}

int32_t Adafruit_BMP085_Unified::readB5() {
	int32_t ut, b5;
	readRawTemperature(&ut);
	return computeB5(ut);
}

float Adafruit_BMP085_Unified::convertB5Temp(int32_t b5) {
	float t;
	t = (b5 + 8) >> 4;
	t /= 10;
